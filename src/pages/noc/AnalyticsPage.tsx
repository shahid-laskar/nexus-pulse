...
const levelColors = {
  ...
  FATAL: 'text-white bg-rose-600 border-rose-700',
}
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <PageHeader
        ...
        subtitle="Real-time Docker container telemetry from edge nodes"
        actions={
        ...
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-4">
          ...
          <div className="xl:col-span-1 space-y-4">
            <Panel title="Queries" description="Quick LogQL filters">
              <div className="space-y-2">
                {COMMON_QUERIES.map(cq => (
                  <button
                    ...
                    className={cn(
                      "w-full text-left px-3 py-2 rounded-lg text-[12px] font-medium transition-colors border",
                      query === cq.query 
              ...
              <div className="mt-6 pt-4 border-t border-slate-200">
                <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-3">Live Streaming</p>
                <Button 
                ...
                {isLive && <p className="text-[10px] text-slate-500 mt-2 text-center animate-pulse">Polling every 3s...</p>}
              </div>
            </Panel>

            <Panel title="Log Summary" description="Overview of current view">
               <div className="flex justify-between items-center py-2 border-b border-slate-200">
                 <span className="text-slate-500 text-[12px]">Lines returned</span>
                 <span className="font-mono text-slate-900 text-[12px] font-semibold">{logs.length}</span>
               </div>
               <div className="flex justify-between items-center py-2 border-b border-slate-200">
                 <span className="text-slate-500 text-[12px]">Errors</span>
                 <span className="font-mono text-rose-600 text-[12px] font-semibold">{logs.filter(l => l.level === 'ERROR' || l.level === 'FATAL').length}</span>
               </div>
               <div className="flex justify-between items-center py-2">
                 <span className="text-slate-500 text-[12px]">Warnings</span>
                 <span className="font-mono text-amber-600 text-[12px] font-semibold">{logs.filter(l => l.level === 'WARN').length}</span>
               </div>
            </Panel>
          </div>
          ...
          <div className="xl:col-span-3">
            <Panel title="Log Explorer" className="h-[calc(100vh-140px)] flex flex-col">
              ...
              {/* Log Output Area */}
              <div className="bg-[#0f1219] rounded-lg border border-slate-200 flex-1 overflow-auto p-1 font-mono text-[11.5px] leading-relaxed text-gray-300 relative shadow-inner">
